...
int safeSend(int clientSocket, const uint8_t *buffer, size_t bufferSize)
{
    ...
    printf("\n");
    #endif
	int written = send(clientSocket, buffer, bufferSize, 0);
	if (written == -1) {
        close(clientSocket);
        perror("send failed");
        return EXIT_FAILURE;
...
void clientWorker(int clientSocket)
{
    ...
    #define prepareBuffer frameSize = BUF_LEN; bzero(buffer, BUF_LEN);
    #define initNewFrame frameType = WS_INCOMPLETE_FRAME; readedLength = 0; bzero(buffer, BUF_LEN);
    
	while (frameType == WS_INCOMPLETE_FRAME) {
		int readed = recv(clientSocket, buffer+readedLength, BUF_LEN-readedLength, 0);
		if (!readed) {
			close(clientSocket);
			perror("recv failed");
            return;
